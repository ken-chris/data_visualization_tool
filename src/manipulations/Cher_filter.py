...
class ChernavFilterManipulation(DataManipulation):
    name = "Chernav Filter"
    description = "Apply a Chernav filter to the selected region."
    options = {
        "method": {
            "type": "dropdown",
            "label": "Method",
    ...
    def apply(self, data, timestamps, channel_idx, region, option_values):
        ...
        b, a = cheby1(order, 0.5, normalized_cutoff, btype=method.lower())
